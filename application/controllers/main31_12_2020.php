...
// $this->load->view('home_content_view');
    ...
    public function recover_password(){
        ...
        if($checkData){
            ...
            // $message_body = 'Dear '.$selectdata->name.'<br><br>'.$otp.' is your otp. Plese do not share with Anyone'.'<br><br><br>Regards<br>Team Skillpromise';
            // $bcc = '';
            
            // $data['mail_to'] = $selectdata->mail;
            // $data['subject'] = "Account Recovery OTP skillpromise.com";
            ...
            // // curl init //
            // $ch = curl_init();
            // $url = "http://mailer.lexiconcpl.com/sendGrid/";
            ...
            $data['send_sms_Message'] = 'Dear ' . $selectdata->name.'<br>'.$otp.' is Your one time password (OTP). Please enter the OTP to reset Account.<br> Thank You,<br>Team Skill Promise.$@#@$';
...
//                        $this->load->view("manage-payment_details", $details);
                    ...
                    // case "node":
                    //     // echo 'Hello';die;
                    //     $query['data'] = $this->main_model->select($table,'*',"name","");
                    //     echo $this->db->last_query();
                    //     echo "<pre>";
                    //     print_r($data);
                    //     die;
        ...
        if (isset($_FILES['image_file']) && !empty($_FILES['image_file']['name'])) {//managing image upload
            ...
            switch ($table) {//separate image directory for user photos
                ...
                case "packages":
                    ...
                    //$this->saveImage('./assets/img/uploads/packages/', $table . '-' . $id . '.' . $extn);
                    break;
                case "package_category":
                    
                        $this->saveImage('./assets/img/uploads/package_category/', $table . '-' . $id . '.' . $extn);
                        $data['id'] = $id;
                        $data['image_file'] = $table . '-' . $id . '.' . $extn;
                        $id = $this->main_model->add_update_record($table, $data, 'id');
                ...
                case "customer":
                        ...
                        $this->saveImage('./assets/img/uploads/customer/', $table . '-' . $id . '.' . $extn);
                        $data['id'] = $id;
                        $data['image_file'] = $table . '-' . $id . '.' . $extn;
                        $id = $this->main_model->add_update_record($table, $data, 'id');
                ...
                default:
                        ...
                        $this->saveImage('./assets/img/uploads/', $table . '-' . $id . '.' . $extn);
                        $data['id'] = $id;
                        $data['image_file'] = $table . '-' . $id . '.' . $extn;
                        $id = $this->main_model->add_update_record($table, $data, 'id');
...
//                $this->load->view('show_node', $row_data);
    ...
    public function self_introduction_data_pdf(){
        ...
        if($CheckData){
            ...
            $html ='<table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                    <td align="" valign="middle" style="font-size:16px;">Self-Introduction Action Sheet<hr></td>
                </tr></table>';
            $pdf->writeHTML($html, true, false, true, false, '');
...
//        }
    ...
    public function api_node($start_from,$end_from){
        
        $data['start_from'] = $start_from;
        $data['end_from'] = $end_from;
        
        // curl init //
        $ch = curl_init();
        $url = "https://skillpromise.com/index.php?/node_api";
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        ...
        foreach($node_data as $key=> $value){
            ...
            $insert_data = array(
                ...
                'type' => $value->type,
                'image_file' => (isset($value->image_file)?$value->image_file:''),
                'duration_from' => (isset($value->duration_from)?$value->duration_from:''),
                'duration_to' => (isset($value->duration_to)?$value->duration_to:''),
                'status' => (isset($value->status)?$value->status:'Inactive'),
                'created_by' => (isset($value->created_by)?$value->created_by:'0'),
                'created_date' => (isset($value->created_date)?$value->created_date:'2020-12-14 15:15:15'),
                'modify_by' => (isset($value->modify_by)?$value->modify_by:'5'),
                'modify_date' => (isset($value->modify_date)?$value->modify_date:'5'),
                'is_deleted' => (isset($value->is_deleted)?$value->is_deleted:'0')
                );
                
                $this->main_model->add_update_record('node',$insert_data);
        }
