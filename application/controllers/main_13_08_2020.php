...
//                    $this->main_model->add_update_record('sheet_section', $update_data, 'section');
                ...
                foreach ($_POST['impovement'] as $id => $value) {
                    //foreach ($value['action_value'] as $key => $value_action) {
                        ...
                        $data['action_values'] = $value['action_value'][0];
                        $return_id = $this->main_model->add_update_record($table, $data);
                    //}
                }
                unset($_POST);
                        ...
                        $html ='<table width="100%" cellpadding="0" cellspacing="0" border="0">
                            ...
                            //================================================================//
                               foreach ($data['listing_type_data'] as $value) {
                                    ...
                                    $html = '<p style="color:#4cae4c;font-size:10px;">'.$value["header_name"].'</p>';
...
//    }
    ...
    public function printPdf(){
       ...
       // $data ="hello";
        ...
        // for Internships
        if (isset($data['internship']) && ($data['internship'] == '1')) {
            $html ='<h4>Internships</h4><br><hr><br><br>';
            $pdf->writeHTML($html, true, false, true, false, '');
            
            if(!empty($data['int'])){
               
                //foreach($data['int'] as $vkey){
                    $html ='';
                    $html .='<table width="100%" cellpadding="0" cellspacing="0" border="0">';
                    $html .='<tr>
                        <td style="padding: 0.5px 0.5px 0.5px 0"><strong>Name of the Organization:</strong> '.$data['int']['org'].'</td>
                        <td style="padding: 0.5px 0.5px 0.5px 0"><strong>Duration:</strong> '.$data['int']['duration'].'</td>
                      </tr>';
                    $html .='<tr>
                        <td style="padding: 0.5px 0.5px 0.5px 0"><strong>Designation:</strong> '.$data['int']['designation'].'</td>
                        <td style="padding: 0.5px 0.5px 0.5px 0"><strong>Location:</strong> '.$data['int']['loc'].'</td>
                      </tr>';
                    $html .='</table>';
                    
                    $html .='<p style="text-align: justify !important;"><strong>Project Title:&nbsp;</strong>'.((!empty($data['int']['project']) ? $data['int']['project'] : "")).'</p>';
                    
                    $html .='<p style="text-align: justify !important;"><strong>Methodology:&nbsp;</strong>'.((!empty($data['int']['method']) ? $data['int']['method'] : "")).'</p>';
                    
                    if(!empty($data['int']['Responsibility'])){
                        $html .='<h4>Key Responsibilities</h4>';
                        $html .='<ul>';
                        foreach($data['int']['Responsibility'] as $rkey => $rvalue){
                            $html .='<li>'.$rvalue['data'].'</li>';
                        }
                        $html .='</ul>';
                    }
                    
                    if(!empty($data['int']['Achievement'])){
                        $html .='<h4>Achievement</h4>';
                        $html .='<ul>';
                        foreach($vkey['Achievement'] as $rkey => $rvalue){
                            $html .='<li>'.$rvalue['data'].'</li>';
        ...
        if(!empty($draft_data)){
            if(!empty($data['pdf_data_from_sheets'][0]->pdf_header)){
                ...
                foreach($prev_content as $pkey => $pvalue){
                    $html ='<h4 style="color:#4cae4c;">'.$pkey.'</h4>';
                    $html .= '<ol>';
