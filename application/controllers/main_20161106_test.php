...
//                    print_r($arr);die;
                ...
                } else if ($section_type_id == 12) {//Report for "Add More checkbox content"
                    $request_fields = array("*");
                    $filter[0]['id'] = 'user_id';
                    $filter[0]['value'] = $_SESSION['user_id'];
                    $filter[1]['id'] = 'sheet_id';
                    ...
                    $key_values = $this->main_model->get_many_records('cover_letter', $filter, $request_fields, 'id');
                    $result['data'] = $key_values;
//                    $result['sheet_name'] = $this->main_model->get_name_from_id('sheets','name',$sheet_id,'id');
//                    echo '<pre>';
//                    print_r($result['data']);die;
...
//                    print_r($arr);
//                    die;
                    $this->load->view('analytics_for_cover_letter', $arr);
                } else {
...
//                    print_r($section_id);die;
                ...
                case 'cover_letter':
                    $records['data'] = $this->main_model->get_records_from_id('cover_letter', $_SESSION['user_id'], 'user_id');
                    $this->load->view("draft_cover_letter", $records);
                    break;
...
//        }
    ...
    public function cover_letter_mail($mail_data = '') {
        $name = $this->main_model->get_name_from_id("users", "name", $_SESSION['user_id'], "id");
        ...
        $message_body = $mail_data['salutation'] . ' ' . $name . ',<br><br>' . $mail_data['paragraph1'] . "<br>" . $mail_data['paragraph2'] . "<br>" . $mail_data['paragraph3'] . "<br>" . $mail_data['closing'] . "<br><br>" . $mail_data['signature'];
        $this->email->from($mail_data['sent_from']);
        $this->email->to($mail_data['sent_to']);
...
//                print_r($data);die;
            ...
            case 'cover_letter':
                $data['template_instruction_id'] = $id;
                $this->load->view('add-cover_letter', $data);
                break;
...
//                    print_r($data);die;
                ...
                break;
            case 12: // Cover Letter --Manage
                $req = array('*');
                $fil[0]['id'] = 'template_instruction_id';
                $fil[0]['value'] = $template_instruction_id;
                $data['cl_data'] = $this->main_model->get_many_records('cover_letter', $fil, $req, '');
                $data['template_instruction_id'] = $template_instruction_id;
//                    echo '<pre>';
//                    print_r($data);die;
                $this->load->view('manage-cover_letter', $data);
                break;
        }
    }
...
//----check for add or edit UI call--
                    ...
                    $fil[2]['id'] = 'is_submit';
                    $fil[2]['value'] = '1';
                    $draft_data = $this->main_model->get_many_records('cover_letter', $fil, array('*'), 'id');

                    if (!empty($draft_data)) { //     draft/edit UI call
//                        echo '<pre>';
//                        print_r($draft_data);die;
                        $data['draft_data'] = $draft_data;
                        $data['draft_data'][0]['instruction'] = $this->main_model->get_name_from_id('template_instruction', 'instruction_text', $active_sheet);
                        ...
                        foreach ($data['draft_data'] as $key => $category) {
                            ...
                            $filter[1]['id'] = 'section_id';
                            $filter[1]['value'] = $category['section_id'];
                            $data['cat_group'][$category['section_id']] = $this->main_model->get_many_records("cover_letter", $filter, $req, 'id');
                        }
//                        echo '<pre>';
//                        print_r($data);
//                        die;
                        $this->load->view($action . 'add-cover_letter', $data);
                    }
//----end check for add or edit UI call--
                    else {
//                        echo '<pre>';
...
//            $email = $_POST['email'];
            ...
            $message_body = 'Dear' . ' ' . $first_name
                    . ',<br><br>' . 'Thank you for signing up for our Email Newsletter.Click on the link below to confirm your subscription<br>' . '<a href="' . base_url() . 'mailSubscribed/' . $return_id . '" style="background-color:#32CD32;border: none;
...
color: white;
 ...
 margin: 4px 2px;
 border-color: #00897b;
 cursor: pointer;">Confirm</a>' . '<br><br>' . 'Please note that clicking on the link above will take you to a page where you can download the “Art of Building Credibility e-Book” as your subscription bonus by clicking on the button “Download”.Get in touch with us at customercare@skillpromise.com, if you have any Query, Suggestion or Request.Thank you again for subscribing to our Email Newsletter. Enjoy using Skillpromise.com programs!<br><br>Regards<br> Team Skillpromise <br> <br>To unsubscribe click on the button below <br> ' . '<a href="' . base_url() . 'mailUnSubscribed/' . $return_id . '" style="background-color:#ffbf00;border: none;
 color: white;
 padding: 10px 20px;
 text-align: center;
 text-decoration: none;
...
//            $this->load->view('print_receipt', $update_payment);
    ...
    public function automatic($package = 0, $reg = 0) {
        $cart_details = $this->cart->contents();
        foreach ($cart_details as $key => $value) {
            $amount['total_amt'] = ($value['subtotal'] + (($value['subtotal'] * 12) / 100));
        }
    ...
    public function generate_word() {
        ...
        //putenv('TMPDIR=' . base_url() . 'assets/');
        //ini_set('upload_tmp_dir', 'C:\wamp64\skillpromise_live\assets');
...
// add style settings for the title and paragraph
        ...
        $data = $this->main_model->get_records_from_id('cover_letter', $_SESSION['user_id'], 'user_id');
        $section = $phpWord->addSection();
        $section->addText("To : " . $data["sent_from"]);
        $section->addTextBreak(1);
        $section->addText("From : " . $data["sent_to"]);
        $section->addTextBreak(1);
        $section->addText("Subject : " . $data["subject"]);
        $section->addTextBreak(1);
        $section->addText($data["salutation"] . " " . $_SESSION['user_name']);
        $section->addTextBreak(1);
        $section->addTextBreak(1);
        $section->addText($data["paragraph1"]);
        $section->addTextBreak(1);
        $section->addText($data["paragraph2"]);
        $section->addTextBreak(1);
        $section->addText($data["paragraph3"]);
        $section->addTextBreak(1);
        $section->addText("Best Regards");
        $section->addTextBreak(1);
        $section->addText($data["closing"]);
        $section->addTextBreak(1);
        $section->addTextBreak(1);
        $section->addText($data["signature"]);
...
// send results to browser to download
        ...
        exit;
//        header('Location: ' . base_url() . "manage/cover_letter");
    }
