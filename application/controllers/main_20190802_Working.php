...
//                    echo '<pre>';
//                    print_r($data);die;
                $this->load->view('manage-addmore_checkbox_category', $data);
                break;
        }
    }
...
//----check for add or edit UI call--
                    ...
                    $data['cat_label'] = $this->main_model->get_name_from_id("template_instruction", "custom_label1", $active_sheet, "id");
                    $data['act_label'] = $this->main_model->get_name_from_id("template_instruction", "custom_label2", $active_sheet, "id");
...
//    for payment gateway
    public function requestMerchant($package = 0, $reg = 0) {
//        $query = $this->main_model->get_records('packages', 'id', $package);
//        $row_data = $query->row();
        $_POST['product'] = "NSE";
        $_POST['TType'] = "NBFundTransfer";
        $_POST['clientcode'] = '007';
        $_POST['AccountNo'] = "1234567890";
        $_POST['ru'] = base_url() . "payment_details/" . $package . "/" . $reg;
// $payment = new payment();
...
//        $postFields .= "&amt=" . $this->cart->total() + (($this->cart->total() / 100) * 18);
        $postFields .= "&amt=999";
        ...
        $postFields .= "&udf3=99999999999999";
        $postFields .= "&udf2=jyoti.chopra@lexiconcpl.com";
...
//curl_setopt($ch, CURLOPT_SSLVERSION,3);
    ...
    public function payment_details($package = 0, $reg = 0) {
        $_POST['atom_txn_id'] = $_POST['mmp_txn'];
        $_POST['email'] = $_POST['udf2'];
        $_POST['mobile'] = $_POST['udf3'];
        $_POST['card_number'] = $_POST['CardNumber'];
        $_POST['date'] = date("Y-m-d H:i:s", strtotime($_POST['date']));

        if (isset($_POST['desc'])) {
            $_POST['description'] = $_POST['desc'];
        ...
        if (!empty($return_data) && $_POST['f_code'] == 'Ok') {
            $cart_details = $this->cart->contents();
...
// $url = "http://166.62.121.164/lexi-mailer/send-mail-V-2.php"; // where you want to post data
    ...
    public function registrationMail_student($mail_data) {
        ...
        $filter[0]["value"] = $user_package_details["users"];
        $req = array("package");
        $packages = $this->main_model->get_many_records("users-package", $mail_data, "id");
        $package_name = "";
        $i++;
        foreach ($packages as $key => $value) {
            $package_name .= $i . ". " . $this->main_model->get_name_from_id("packages", "name", $value["package"], "id") . "<br>";
...
//        print_r($users_details);
        if ($unique_code == $users_details['mail_confirm_token']) {
            ...
            $payment_id = $this->main_model->get_name_from_id("users-package", 'id', $user, "users");
...
//        echo '<div class="modal fade" id="newLetter_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">';
...
//                echo '<h4>Thank you for subscribing to our Email Newsletter. Please follow the following steps to complete your signup process:</h4>';
//                echo '<ul class="custom">';
//                echo '<li>Check for our Email in your Inbox and in case it is not there, check your Spam folder.</li>';
//                echo '<li>Click the link “Confirm” in the Email that you will receive from us.</li>';
//                echo '<li>Download your Art of Building e-Book.</li>';
//                echo '</ul>';
//                echo '</div>';
