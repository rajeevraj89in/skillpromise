...
class main extends CI_Controller {
    ...
    public function __construct() {
        ...
        $exempted_controllers = array('category','mailUnSubscribed','mailSubscribed','checkout','blogComment','SaveBlogComment','blogDetail', 'blogList','show_sections', 'index', 'signin', 'change_password', 'do_password_change', 'login_handler', 'logout', 'set', 'update', 'set_many', 'ajax_feeder', 'ajax_fetch', 'loadGrid', 'saveImage', 'submit', 'add_request', 'node', 'score', 'subscribNewsLetter', 'registrationMail', 'register', 'value_action', 'get_records_group_by', 'manage', 'showsheet', 'edit', 'delete', 'set_comments', 'get_comments', 'set_like', 'set_dislike', 'like_counts', 'add', 'quiz', 'create', 'set_data', 'manage_sheet_values', 'sheets', 'testing_view', 'remove', 'skill_talk_slider', 'slider', 'skill-talk-slider', 'assessment-center-slider', 'employability-zone-slider', 'about-us', 'contact-us', 'privacy-policy', 'terms-of-use', 'our-values', 'news-letter', 'our-benefits', 'add_excel_user', 'upload_application', 'execl_upload', 'is_validate_application', 'home_sheets', 'set_data_home', 'sana-for-school', 'sana-for-higher', 'sana-for-professionals', 'learning-objectives-school', 'program-benefits-school', 'course-content-school', 'learning-objectives-higher', 'program-benefits-higher', 'course-content-higher', 'learning-objectives-professional', 'program-benefits-professional', 'course-content-professional', 'blog', 'blog-detail', 'my-cart', 'blog_main', 'datetime', 'blog_nav', 'add_blog', 'manage', 'add_cart', 'my_cart', 'remove_cart', 'analytics');
...
//========================================================================
        switch ($quiz_mode) {
            ...
            case "result" :
                ...
                $filter2[2]['value'] = '0';
                $request_fields = array("selected as selected_opt");
...
//                if ($is_packages) {
//                    header('Location: ' . base_url() . 'value_action/sheets/' . $row_data->quiz_id . '/1' . '/' . $row_data->nav_type . '/' . $row_data->id);
//                } else {
//                    header('Location: ' . base_url());
//                }
                break;
...
//====================Subnodelist=========================================
   public function subnodeslist($node = 0) {
        ...
        $req = array('id', 'name');
        $data = $this->main_model->get_many_records('node', $filter, $req);

        $sql = "SELECT q.scored_marks,n.name from node n,quiz_result q WHERE n.id=q.quiz_id AND q.user_id=$user_id AND n.parent_id=$node";

        $dataw = $this->db->query($sql)->result_array();
        $key = 'scored_marks';

        $sum = array_sum(array_column($dataw, $key));
