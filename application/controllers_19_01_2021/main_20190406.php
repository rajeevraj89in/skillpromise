...
//====================Subnodelist=========================================
    public function subnodeslist($node = 0) {
        ...
        $sql = "SELECT q.scored_marks,n.name from quiz n,quiz_result q WHERE n.id=q.quiz_id AND q.user_id=$user_id AND quiz_id=$node";

        $dataw['scored_marks'] = $this->db->query($sql)->result_array();

        $key = 'scored_marks';
        $searchdata = $this->array_column_recursive($dataw, $key);
        $sum = array_sum($searchdata);
