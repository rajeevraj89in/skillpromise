...
//            die;
    ...
    function is_sheet_submited($sheet_id = 0) {
        ...
        $result = $CI_Model->main_model->get_many_records("action_sheets", $filter, $req,''); 
...
// Return object
    ...
    function get_side_navigation_panel_new($node = "", $nav_type = '', $order_by = "name") {//For node system	
        $CI_Model = & get_instance();
        $CI_Model->load->model('main_model');
        $quiz = '<div class="panel panel-primary border">
            <!--<div class="panel-heading">-->
                <h4 class="quickpanelhead quiz_head">Quiz</h4>
            <ul class="nav nav-pills nav-stacked nav-sidebar" id="accordion">';
        
        $sheet = '<div class="panel panel-primary border">
            <!--<div class="panel-heading">-->
                <h4 class="quickpanelhead quiz_head">Sheet</h4>
            <ul class="nav nav-pills nav-stacked nav-sidebar" id="accordion">';
...
//                $is_permitted = $this->is_permitted($rec->id);
//                if ($is_permitted) {

                    switch ($node_type) {
                        case "Module":
                            $module.= '<li><a href="' . base_url() . "node/$nav_type/$rec->id" . '">' . "$rec->name</a></li>";
                            $module_count++;
                            break;
                        case "Sheet":
                            ...
                            $sheet.= '<li><a href="' . base_url() . "value_action/sheets/$sheet_id" . '">' . "$rec->name</a></li>";
                            $sheet_count++;
                            break;

                        case "Quiz":
                            $is_quiz_attached = $CI_Model->main_model->get_name_from_id('quiz', 'id', $rec->id, "node");
                            $quiz_id = $CI_Model->main_model->get_name_from_id('quiz', 'id', $rec->id, 'node');
                            $quiz_type = $CI_Model->main_model->get_name_from_id('quiz', 'quiz_type', $quiz_id, "id");

                            //$quiz_type = 'Practice';&& $nav_type =='program' && $quiz_type == 'Practice'
                            if ($is_quiz_attached && $nav_type == 'program' && $quiz_type == 'Practice') {
                                // $quiz.= '<li><a href="' . base_url() . "node/" . get_quiz_type_from_node($rec->id) . '">' . "$rec->name</a></li>";
                                $quiz .= '<li><a href="' . base_url() . "node/$nav_type/$rec->id" . '">' . "$rec->name</a></li>";
                                $quiz_count++;
                            }
                             if ($is_quiz_attached && $nav_type == 'program' && $quiz_type == 'Subjective') {
				//echo "testing";
                                // $quiz.= '<li><a href="' . base_url() . "node/" . get_quiz_type_from_node($rec->id) . '">' . "$rec->name</a></li>";
                                $quiz .= '<li><a href="' . base_url() . "node/$nav_type/$rec->id" . '">' . "$rec->name</a></li>";
                                $quiz_count++;
                            }
                            if ($is_quiz_attached && $nav_type == 'test_centre' && $quiz_type != 'Practice') {
                                // $quiz.= '<li><a href="' . base_url() . "node/" . get_quiz_type_from_node($rec->id) . '">' . "$rec->name</a></li>";
                                $quiz .= '<li><a href="' . base_url() . "node/$nav_type/$rec->id" . '">' . "$rec->name</a></li>";
                                $quiz_count++;
                            }
...
//         $CI_Model = & get_instance();
//        //$question_id=292;
//        $total_likes = $CI_Model->main_model->get_record_count('likes_dislikes', 'records', 1);
