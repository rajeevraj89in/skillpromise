...
//Purpose : Display The Ui content of "Range Type " Section/T.I.
?>
<?php
    ...
    <div class="row text-center">
        <div class="col-md-12">
            <?php if (!empty($template_instruction_id)) { ?>
                <a class="btn btn-primary" href="<?php echo base_url() . 'create/range_type/' . $template_instruction_id; ?>">Add sheet content</a>
            <?php } else { ?>
                <a class="btn btn-primary" href="<?php echo base_url() . 'create/range_type/'; ?>">Add sheet content</a>
            <?php }
            ?>
        </div>
    </div>
                    ...
                    <tbody>
                        ...
                        foreach ($data as $rec) {
                            $tbl_str.='<tr class="post">'
                                    . "<td>" . $count++ . "</td>"
                                    . "<td>" . $rec['name'] . "</td>"
                                    . "<td>" . $rec['normal_range'] . "</td>"
                                    . "<td>" . $this->main_model->get_name_from_id('actionsheet_category','name',$rec['category'] ). "</td>"
                                    . '<td class="text-center"><a href="' . base_url() . "edit/range_type/" . $rec['id'] . '"><span class="glyphicon glyphicon-pencil"></span></a></td>'
                                    . '<td class="text-center"><a href="' . base_url() . "delete/range_type/" . $rec['id'] . '"><span class="glyphicon glyphicon-trash"></span></a></td>'
                                    . "</tr>";
                        }
                        echo $tbl_str;
                        ?>
