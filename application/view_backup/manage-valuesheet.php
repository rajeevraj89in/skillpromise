...
<section class="col-md-9">
    <h1 class="page-header"> SHEET </h1>
    <!--Add question btn-->
<!--    <div class="row text-center">
        <div class="col-md-12">
            <a id="addLink" href="<?php// echo base_url() . 'add/node'; ?>" class="btn btn-primary">Add Node</a>
        </div>
    </div>-->
    <hr />
...
<script>
    ...
    function fillGrid(JSONtext, gridID) {
        ...
        $.each(JSONobject, function (i, item) {
            ...
            //td = $("<td>" + item.node + "</td>");
            //tr.append(td);
           // td = $("<td>" + item.details + "</td>");
            //tr.append(td);
            td = $("<td>" + item.status + "</td>");
            tr.append(td);
            td = $('<td class="text-center"><a href="' + base_url + 'value_action/sheets/' + item.id + '"><span class = "glyphicon glyphicon-eye-open" ></span></a></td>');
            tr.append(td);
            //td = $('<td class="text-center"><a href="' + base_url + 'delete/node/' + item.id + '"><span class = "glyphicon glyphicon-trash" ></span></a></td>');
            //tr.append(td);
        });
    }
...
//            alert(level);
    ...
    $(document).ready(function () {
        $("#show").click(function () {
            ...
            initiateAjax('sheets', 'node', getValue, ['id', 'name','node', 'status'], fillGrid, "content");
        });
    });
