...
<section class="col-md-9">
    ...
    <form class="form-inline" role="form">
        <div class="row">
            <div class="form-group space-bootom">
                <label for="" class="col-sm-5 control-label">Select College</label>
                <div class="col-sm-7">
                    <?php //$data = $this->main_model->select_college('users','scu'); ?>
                    <select class="form-control" name="scu" id="scu">
                        <option value="">Select College</option>
                        <?php $data = $this->main_model->select_college('users','scu');
                            foreach($data as $key=>$value){
                                if($value['scu'] != null){
                                ?>
                                <option value="<?php echo $value['scu']; ?>"><?php echo $value['scu']; ?></option>
                                <?php
                            }
            ...
            <div class="form-group space-bootom">
                <label for="" class="col-sm-5 control-label">Select Course</label>
                <div class="col-sm-7">
                    <?php //$data = $this->main_model->select_college('users','scu'); ?>
                    <select class="form-control" name="course" id="course">
                        <option value="">Select Course</option>
                        <?php $data1 = $this->main_model->select_college('users','course');
                            foreach($data1 as $key1=>$value1){
                                if($value1['course'] != null){
                                ?>
                                <option value="<?php echo $value1['course']; ?>"><?php echo $value1['course']; ?></option>
                                <?php
                            }
            ...
            <div class="form-group space-bootom">
                <label for="" class="col-sm-5 control-label">Select Stream</label>
                <div class="col-sm-7">
                    <?php //$data = $this->main_model->select_college('users','scu'); ?>
                    <select class="form-control" name="stream" id="stream">
                        <option value="">Select Stream</option>
                        <?php $data1 = $this->main_model->select_college('users','stream');
                            foreach($data1 as $key1=>$value1){
                                if($value1['stream'] != null){
                                ?>
                                <option value="<?php echo $value1['stream']; ?>"><?php echo $value1['stream']; ?></option>
                                <?php
                            }
            ...
            <div class="col-md-12">
                <div id ="select_level_outer">
                    ...
                    <div class="form-group space-bootom level-div1" id="leveldiv1">
                        <label for="" class="col-sm-4 control-label">Level-1</label>
                        <div class="col-sm-8">
                            <?php //$data = $this->main_model->select_college('users','scu'); ?>
                            <select class="form-control node-select" id="level1" name="node" required>
                                <?php echo $this->main_model->summry_report(); ?>
                            </select>
                        </div>
                    </div>
                </div>
    ...
    function fillGrid(JSONtext, gridID) {
        ...
        $.each(JSONobject, function (i, item) {
            if (item.type == 'Quiz') {
                ...
                tr.append(td);
                td = $('<td class="text-center"><a href="' + base_url + 'SummaryReport/' + item.id + '/'+scu+'/'+course+'/'+stream+'" class="btm btn-sm btn-primary">Summary Report</a></td>');
                tr.append(td);
                td = $('<td class="text-center"><a href="' + base_url + 'QuestionReport/' + item.id + '" class="btm btn-sm btn-primary">Question Wise Report</a></td>');
                tr.append(td);
            }
        });
    }
