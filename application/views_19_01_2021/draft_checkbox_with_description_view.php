<?php
//Created:Dewangshu , 24/08/2017 for edit/Save as Draft data of "Checkbox with tooltip
            ...
            <form id="mainForm" enctype="multipart/form-data" class="form-horizontal" role="form" action="<?php echo base_url() . 'set_data/action_sheets'; ?>" method="POST">
                ...
                foreach ($category_id as $category) {
                    echo '<div class="panel panel-success" style="border-color: #fff; border-radius: 0; box-shadow: none; border: none;">
                     <div class="panel-heading" style="color: #ffffff; background-color: #01897b;  border-color: #01897b;  margin-bottom: 1rem;">
                ...
                <table class="table table-bordered" id="">
                    ...
                    foreach ($data[$category['category']] as $rec) {
                        ...
                        $i++;
                        //---for previous saved data----
                        $ch = 0;
                        $prev_content = '';
                        foreach ($draft_data as $key_dr => $val_draft) {
                            if ($val_draft['sheetvalue_id'] == $rec['id']) {
                                $ch = 1;
                                $prev_content = $val_draft['reason'];
                                break;
                            }
                        }
                        //----
                        if ($ch) {
                            ...
                            echo '<td width="33.33%" style="background: #ffffff; padding: 0 0px 0 9px; margin: 0; border: 1px inset #01897b;"><input  border-radius: 6rem;" data-content="' . $rec['description'] . '" type="checkbox" checked class="check"  data-category="' . $category['category'] . '" id="' . $rec['id'] . '" name="sheetvalue_id[' . $rec['id'] . ']" value="' . $prev_content . '">'
                            . '<span style="display: inline-block; padding: 12px 10px; width: 91.4%; background: #01897b;  margin: 0px 0 0 10px; color: #fff;" class="tool" data-toggle="tooltip" data-name="'.$rec['name'].'" title="Click me to know more" data-content="' .  htmlspecialchars($rec['tool_tip']) . '"> '
                            . $rec['name'] . '</span></td>';
//                                echo '<td>'.$prev_content.'</td>';
                        } else {
                            echo '<td width="33.33%" style="background: #ffffff; padding: 0 0px 0 9px; margin: 0;  border: 1px inset #01897b;"><input data-content="' . $rec['description'] . '" type="checkbox" class="check" data-category="' . $category['category'] . '" id="' . $rec['id'] . '">'
                            . '<span style="display: inline-block; padding: 12px 10px; width: 91.4%; background: #01897b;  margin: 0px 0 0 10px; color: #fff;" class="tool" data-toggle="tooltip" data-name="'.$rec['name'].'" title="Click me to know more" data-content="' .  htmlspecialchars($rec['tool_tip']) . '"> '
                            . $rec['name'] . '</span></td>';
                        }
                ...
                <div class="form-group">
                    <div class="col-sm-12">
                        <center>
                            <button type="submit" value="1" name="draft" class="btn btn-success">Save as Draft</button>&nbsp;<a class="btn btn-success" href="<?php echo base_url('sheets_pdf/sheets/'.$sheet_id); ?>">Download as PDF</a>
                        </center>
...
//  //  $('#importModal').modal('hide');
  ...
  function displayTooltip(content,name) {
//    alert("content");die;
